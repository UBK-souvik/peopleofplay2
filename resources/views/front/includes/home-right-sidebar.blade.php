  ...
  <div class="right-column k_sidebar left-colom-sidebar">
    <div class="DashboardProfile" id="sidebartoggle-container">
      <div class="MobileToggleSideBar">
        @if(isset($feeds_ad))
        @if(@$feeds_ad[3]->type == 'right_ad' && !empty(@$feeds_ad[3]->image))
          <div class="OptionalAd OptionalAd1 text-center mb-4">
            <a href="@if(!empty($feeds_ad[3]->url)){{ $feeds_ad[3]->url }})@else{{'javascript:void(0)'}}@endif" target="@if(!empty($feeds_ad[3]->url)){{ '_blank' }})@endif">
              <img src="{{asset('uploads/images/feeds_ad/'.$feeds_ad[3]->image)}}" alt="">
            </a>
          </div>
        @endif
         @endif
    ...
    beforeSend: function()
    {
        ...
        success: function (data)
        {
          if(data.success == 0){
            ...
            // window.location.replace('{{ route("front.feeds")}}');
            if(data.page == 'home'){
...
function changeType(e) {
  ...
  $.ajax({
   ...
   //   $.ajax({
   //      url: "{{ route('front.feeds.feed_image_upload') }}",
   //      type: 'POST',
   //      processData: false,
   //      contentType: false,
   //      dataType: 'json',
   ...
   //        var image_url = "{{url('/uploads/images/feed/')}}"+'/'+response.image;
