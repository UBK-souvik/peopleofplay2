...
@if(!empty($is_chk_user_blog_news_create_flag))
	if(CKEDITOR)
	{
...
var int_file_size_divisor_new = {{$int_file_size_divisor_new}};
var int_file_size_limit_new = {{$int_file_size_limit_new}};
...
<script type="text/javascript">
  ...
  $(document).on('click', '.delete_account_button', function (e) {
      ...
      var href = "{{url('/delete_account')}}?pass="+password;
      if(password != '') {
...
<script type="text/javascript">
  var report_successfully = '{{ Session::has("report_successfully") }}';
  $( document ).ready(function() {
...
function deleteRoleDataModal(role_id){
	 ...
	 function FileValidation(str_file_name)
	 {
        ...
        if (fi.files.length > 0) {
            ...
            for (const i = 0; i <= fi.files.length - 1; i++) {
                ...
                if (file > int_file_size_limit_new) {					
                    //$('#{{$str_create_update_button_new}}').attr('disabled', true);
                ...
                //} 
                    ...
                    //alert('<b>' + file + '</b> KB');
					//$('#{{$str_create_update_button_new}}').attr('disabled', false);
					return true;
                }
            }
        }
		...
		function shareWikiToFeed(e,id,type){
			$.ajax({
				url: "{{ route('front.shareWikiToFeed') }}",
				type: 'POST',
				headers: {
				'X-CSRF-TOKEN': ajax_csrf_token_new
				},
