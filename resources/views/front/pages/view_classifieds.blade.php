...
<div class="col-md-9 col-lg-10 MiddleColumnFullWidth">
   <div class="ClassifiedsSearchSection">
         ...
         <div class="container">
                  ...
                  <div class="row mt-3">
                     ...
                     <div class="col-md-6">
                        <select class="form-control" onchange="return get_classified_detail(this.value);">
                           <option value="0">{{$all_classified_categories[8]->name}}</option>
                           @foreach($all_classified_categories as $all_classified_categorie_row)                     
                           @if($all_classified_categorie_row->id == 9)
                           @continue;
                           @endif
                           <option @if(!empty($type_id) && $type_id == $all_classified_categorie_row->id) {{'selected'}} @endif value="{{$all_classified_categorie_row->id}}">{{$all_classified_categorie_row->name}}</option>
                           @endforeach
                        </select>
                     </div>
                  </div>
            ...
            <div class="col-md-12 p-0" style="margin-top: 60px">
               <div class="row">
                  ...
                  @if(!empty($all_classified_list) && count($all_classified_list)>0)
                  @foreach($all_classified_list  as $all_classified_list_row)
                     ...
                     <div class="ClassifiedListBox">
                        <div class="row">
                           <div class="col-md-10 col-10">
                              <h3 class="ClassifiedHeading">{{$all_classified_list_row->title}}</h3>
                           </div>
                                 ...
                                 <div class="dropdown socialDropdown">
                                    ...
                                    <div class="dropdown-content1 socialDropdownContent dictionaryShare dropdown-menu">
                                       <ul class="dropSocialShare">
                                          ...
                                          <li><a target="_blank" href="http://www.facebook.com/sharer.php?u={{ url('/pop-classified-details/'.$all_classified_list_row->slug) }}"><i class="fa photo_icon fa-facebook"></i></a></li>
                                          <li><a target="_blank" href="http://twitter.com/share?url={{ url('/pop-classified-details/'.$all_classified_list_row->slug) }}"><i class="fa photo_icon fa-twitter"></i></a></li>
                                          <li><a target="_blank" href="https://www.instagram.com/?url={{ url('/pop-classified-details/'.$all_classified_list_row->slug) }}"><i class="fa photo_icon fa-instagram"></i></a></li>
                                          <li><a target="_blank" href="https://wa.me/?text={{ url('/pop-classified-details/'.$all_classified_list_row->slug) }}"><i class="fa photo_icon fa-whatsapp"></i></a></li>
                                       </ul>
                                    </div>
                                 </div>
                              </div>
                        ...
                        <div style="min-height: 187px;">
                           ...
                           <div  class="mainDiv clearfix" id="short-desc" >
                              <div class="desc  short-desc" style="float: left;">
                                 {!! nl2br(@$all_classified_list_row->description) !!}
                              </div>
                              <a href="{{ url('/pop-classified-details/'.$all_classified_list_row->slug)}}" class="readMore ProfileReadMore" onclick="readMoreBtn(this)"> Read More... </a>
                           </div>
                           @if($int_user_word_length > $int_description_words_length)
                           <div id="d_dots{{$int_classifed_id_new}}"></div>
                           <div id="d_more{{$int_classifed_id_new}}" style="display:none;">
                              <p>{!! @App\Helpers\UtilitiesTwo::limit_words($all_classified_list_row->description, 0, $int_user_word_length)!!} <a data-toggle="modal" class="readMore ProfileReadMore" onclick="d_myFunction_classified({{$int_classifed_id_new}})" id="d_myBtn{{$int_classifed_id_new}}">Read Less...</a></p>
                           </div>
                           @endif
                        </div>
                        ...
                        <div class="text-center mt-2" id="div-classified-application-{{$int_classifed_id_new}}">
                           ...
                           @if(empty($int_is_application_saved_flag)) onclick="return save_classified_applicant_data({{$int_classifed_id_new}}, {{$int_is_free_not_logged_user}});" @else disabled @endif id="click-application-id-{{$int_classifed_id_new}}" class="text-white clickApplicationBtn classifiedApplication-{{$int_classifed_id_new}}">{{$str_classifed_application_new}}</a>
                        </div>
                        ...
                        <div class="text-center mt-3">
                           <p><small>POSTED BY</small> <strong><a href="{{$str_user_url_new}}">{{$str_user_name}}</a> </strong>on {{@App\Helpers\UtilitiesTwo::get_date_from_date_time_data(@$all_classified_list_row->created_at)}}</p>
                        </div>
      ...
      @if(($int_type_of_user ==2) && ($role_type_id == 3 || $role_type_id ==2))
      <div class="text-center">
         <a href="{{ route('front.user.classified.create') }}" class="text-center textPurple submitAdBtn">Submit an Ad for Free!</a>
      </div>
      @endif 
   </div>
   @include('front.includes.join_mailing')
</div>
<input type="hidden" name="hid_dictionary_url_2" id="hid_dictionary_url_2" value="{{ url('/pop-classified-details/'.$all_classified_list_row->slug) }}"> 
@endsection
@section('scripts')
<script>
