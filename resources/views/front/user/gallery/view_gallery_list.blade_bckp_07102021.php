			...
			@foreach($gallery_data as $gallery_data_row)
			   ...
			   @if($gallery_type == 4)
				...
				@elseif ($gallery_type == 2)
								<div class="item draggable-element pr-1 pb-0" data-responsive="" data-src="{{ $str_media_data }}" 
					data-poster="" data-sub-html="{{ $str_bottom_content }}">
					  	<a href="{{ $str_media_data }}">          
						...
						{!! @App\Helpers\UtilitiesTwo::limit_words($thumbnail_title, 0, 5) !!}
						</p>
						</div> --}}
			   ...
			   @else	        
				 <div class="item draggable-element pr-1 pb-0" data-responsive="" 
					data-src="{{@galleryImageBasePath(@$str_media_data)}}" 
					data-sub-html="{{ $str_bottom_content }}">
						<a href="#">
						   <img class="img-responsive img-div-class-new kimg_div_class_new {{$str_known_for_class}}" 
						   src="{{@galleryImageBasePath(@$str_media_data)}}">
						</a>
						<div class="userPoductTitle withoutOverlay pt-1 mb-3"><small>@if(!empty($str_caption_name_snippet)){{$str_caption_name_snippet}}@endif</small></div>
				  </div>
