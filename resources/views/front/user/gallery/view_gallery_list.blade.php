			...
			@foreach($gallery_data as $gallery_data_row)
			    ...
			    @endphp
			   @if($gallery_type == 4)
			 	<a href="javascript:void(0);" onclick="getIMageGallery('{{ $int_gallery_id }}',4,'{{ Auth::guard('users')->user()->id }}',1);">
			   		<div class=" pr-1 pb-0" >
						   <img class="img-responsive img-div-class-new kimg_div_class_new {{$str_known_for_class}}" 
						   src="{{@awardImageBasePath(@$str_media_data)}}" width="170px">
				...
				@elseif ($gallery_type == 2)
					<a href="javascript:void(0);" onclick="getIMageGallery('{{ $int_gallery_id}}','{{ $gallery_link_type }}','{{ Auth::guard('users')->user()->id }}',1);">  
						<div class="pr-1 pb-0" >
						<img class="img-responsive img-div-class-new kimg_div_class_new"
							src="{{$thumbnail}}" width="170px" />
						<div class="userPoductTitle withoutOverlay pt-1 mb-3"><small>@if(!empty($str_caption_name_snippet)){{$str_caption_name_snippet}}@endif</small></div>
				  	</div>
				  	</a>
			   @else	
			   <a href="javascript:void(0);" onclick="getIMageGallery('{{ $int_gallery_id }}','{{ $gallery_link_type }}','{{ Auth::guard('users')->user()->id }}',1);">        
				 <div class=" pr-1 pb-0">
						   ...
						   <img class="img-responsive img-div-class-new kimg_div_class_new {{$str_known_for_class}}" 
						   src="{{@galleryImageBasePath(@$str_media_data)}}" width="170px">
