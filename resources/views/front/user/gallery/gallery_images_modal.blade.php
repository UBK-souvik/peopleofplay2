            ...
            <div class="row mt-2">
               <div class="col-md-12">
                  <div class="GalleryCaption">
                     ...
                     @if(isset($row->caption) && !empty($row->caption)) <span> {{ $row->caption }}</span><br> @endif
                  </div>
                    {{--  @if(isset($row->title) && !empty($row->title)) <label><b>Title :</b></label> <span> {{ $row->title }}</span> <br> @endif --}}
                     ...
                     <div class="GalleryModelButton">
                        <a href="javascript:void(0);" onclick="return openEditGalleryModal('{{ $row->id }}');" class="btn btn-primary btn-sm" > Edit <i class="fa fa-spinner fa-spin st_gallery_loading" style="display: none;"></i></a>
