            ...
            <div class="esTableFilters esChatUser align-items-center">
              <div class="esTableLeft esMsgUser userTopName" style="@if(isset($newUser) && !empty($newUser)){{ '' }} @else display: none; @endif;">
                <div class="esForAns bg-white m-0">
                  ...
                  <h3 class="m-0 pl-3 pr-2 activeUserName"> {{ @$newUser->first_name }} {{ @$newUser->last_name }}</h3> <i class="fa fa-spinner fa-spin st_name_loader" style="display: none;"></i>
                </div>
...
$('.esSideBar .navbar-toggler').click(function() {
  ...
  var newUserActive = $('#newUserActive').val();
   ...
   $(".chat_list a").each(function(index){
     if(newUserActive !=''){
       var activeUser =this.id.split('_');
      if(activeUser[0] == newUserActive){
         var x = $('#message_id').val(activeUser[1]);
     ...
     $(window).on('load', function() {
        $(".esChatLists > li:first-child a").click();
    });

 function getMassageBox(message_id='',receiver_id='',is_click='',userName='',userImage='') {
   var total_massage = $('#chatTotalMsg').val();
   $.ajax({
     url: "{{ route('front.user.messagetext') }}",
     data: {'message_id':message_id,'total_massage':total_massage,'is_click':is_click},
     type: 'GET',
     dataType:'json',
     beforeSend: function () {
     },
     ...
     success: function (data) {
      if(data.refresh_view == true) {
        ...
        $('#totalMassage').val(data.totalMessage);
        var esName = $('#active .esUsName').text();
        var esImage = $('#active .newImage').html();
        $('.activeUserName').text(esName);
        $('.activeUserImage').html(esImage);
      }else if(data.is_click == 1){
        $('.esInsideRightMessage').html(data.view);
        $(".esInsideChatBox").scrollTop($(".esInsideChatBox").prop('scrollHeight')); 
        $('#totalMassage').val(data.totalMessage);
        var esName = $('#active .esUsName').text();
 ...
 function getMassageUserChat(e,message_id='',receiver_id='',is_click='') {
  ...
  // $('.activeUserName').text(esName);
  // $('.activeUserImage').html(esImage);
  getMassageBox(message_id,receiver_id,is_click);
}
...
function getMassageBoxSidebar(mgs_id,no_of_users,totalMassage) {
 ...
 $.ajax({
   url: "{{ route('front.user.message.message_sidebar') }}",
   ...
   success: function (data) {
    if(data.sidebar == "yes") {
      ...
      $('#totalMassage').val(data.totalMassage);
    }
  }
});
}
...
function raedMassage(message_id) {
  $.ajax({
    ...
    success: function (data) {
      ...
      $('#active .badge-danger').html('');
      $('.esUserMsgBox').css("background-color","#f0f0f1");
    }
  });
}
