...
<div class="container popPaySection px-3" @if(!empty($str_session_id_new)) style="display:none;" @endif id="div-delivery-address-id">
<div class="row">
      ...
      <div class="row">
         <div class="col-md-6 paddingTopFourtyFive">
            <div>
               ...
               <p class="mb-0 fontFifteen fontWeightSix">{{@App\Helpers\Utilities::getUserName($user)}}</p>
               <p class="mb-0 fontFourteen">@if(!empty($user->postal_address)){{ $user->postal_address }}@endif</p>
               @if(!empty($user->city) && !empty($user->state))
               <p class="mb-0 fontFourteen">{{ $user->city }} , {{ $user->state }} </p>
               @endif
               <p class="mb-0 fontFourteen">@if(!empty($user->countrydata->country_name)){{ $user->countrydata->country_name }}@endif  @if(!empty($user->zip_code)) {{'-'}} {{ $user->zip_code }}@endif</p>
               <p class="mb-0 fontFourteen">Email : {{ $user->email }}</p>
               <!-- <p class="mb-0 fontFourteen">Phone No : +{{$user->dial_code}} {{ $user->mobile }}</p> -->
            </div>
         </div>
         <div class="col-md-6 paddingTopFourtyFive">
   ...
   <div class="col-md-12 col-xl-6 OrderSummary">
      ...
      <table class="w-100">
         ...
         <tr>
            ...
            <td class="text-right">
               <p class="mb-0">$<span class="membershipprice">{{ (int) $plan->price }}</span><span
                  class="period">/{{ $plan->validity > 360 ? 'year' : 'month' }}</span> </p>
            </td>
         </tr>
         <tr>
            <td>
         ...
         <tr>
            ...
            <td class="text-right">
               <p class="mb-0">$<span id="span-price-discount-id">{{ (int) $plan->price }}</span><span
                  class="period">/{{ $plan->validity > 360 ? 'year' : 'month' }}</span></p>
            </td>
         </tr>
      </table>
            ...
            <div class="form-group">
               <div class="col-sm-12 p-0">
                  ...
                  <div class="PaymentIcons">
                     <img src="{{ asset('front/images/payment-icon/US_standard.png') }}" alt="visa" class="Paymenticons img-fluid">
                  </div>
                  <!-- <ul class="nav PaymentIconDetails">
                     <li class="nav-item">
                       <a class="nav-link" href="#"><img src="{{ asset('front/images/payment-icon/visa.png') }}" alt="visa" class="VisaIcon"></a>
                     </li>
                     <li class="nav-item">
                       <a class="nav-link" href="#"><img src="{{ asset('front/images/payment-icon/mastercard.png') }}" alt="mastercard" class="MasterCardIcon"></a>
                     </li>
                     <li class="nav-item">
                       <a class="nav-link" href="#"><img src="{{ asset('front/images/payment-icon/americanexpress.png') }}" alt="americanexpress" class="AmericanExpressIcon"></a>
                     </li>
                     <li class="nav-item">
                       <a class="nav-link" href="#"><img src="{{ asset('front/images/payment-icon/jcb.png') }}" alt="jcb" class="JcbIcon"></a>
                     </li>
                     </ul> -->
                  <button id="{{$int_plan_id}}" onclick="select_plan(this.id)"  role="link"  type="button" class="btn btnAll my-4 payLegacyBtnnew" > Pay Now </button>
...
<script type="text/javascript">
      ...
      var stripePublishableKey = "{{\App\Models\SiteSetting::get_keys(0)}}";
   ...
   $(document).ready(function(){
    ...
    @if(!empty($str_session_id_new))

     stripeNewLegacyChangedHandler('{{$str_session_id_new}}');
   ...
   /*var handler = StripeCheckout.configure({
    ...
    image: '{{ asset("front/images/paymentLogo.png")}}',//https://stripe.com/img/documentation/checkout/marketplace.png
    locale: 'auto',
