                              ...
                              <div class="dropdown cq-dropdown" data-name='statuses'>
                                 ...
                                 <ul class="dropdown-menu" aria-labelledby="btndropdown">
                                    <ul class="ulDrop">
                                       ...
                                       <li>
                                          <label class="radio-btn"><input type="checkbox"  name="category_filter[]" value="{{$feedsCategory->id}}" class="pe-2" id="categoryFilter">{{ ucwords($feedsCategory->name) }}</label>
                                       </li>
                                       @endforeach
...
<div class="col-md-3 col-lg-3 rightSideSection">
   <div class="popFeedSidebar">
      @php $feed_id = @$feeds[0]['id']; @endphp
      <div class="popFeedPost w-100 py-4 border-bottom d-none d-lg-block {{$i}}" id="feed-main-box{{$feed_id}}">
         <input type="hidden" class="i_position" value="{{$i}}">
         <div class="feedCategory d-flex pb-2">
            <h6><span><b>MEME OF THE DAY</b></span></h6>
         </div>
...
<script>
   ...
   function newsFeedsFormSubmit(){
      ...
      var formData = new FormData(form_ser);
      $.ajax({
         url:"{{ route('front.news_feeds_filter') }}",
         headers: {
            'X-CSRF-TOKEN': ajax_csrf_token_new
         },
         type:"POST",
   ...
   function submit_news_feeds_form(e){
      $.ajax({
         url:"{{ route('front.submit_news_feeds_form') }}",
         headers: {
            'X-CSRF-TOKEN': ajax_csrf_token_new
         },
         type:"POST",
