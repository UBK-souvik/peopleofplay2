  ...
  <div class="col-ting my-4">
    <div class="control-group file-upload cropie-demo" id="file-upload1">
      <div class="image-box text-center"  >
        <div class="image-box-text @if(!empty(@$news_Feeds->image)){{ 'edit_image_preview' }}@endif">
          <div class="popFeedIcon">
            <img src="{{ asset('front/images/feed/feed_medias.png') }}" class="img-fluid mb-1">
          </div>
        ...
        @if(!empty(@$news_Feeds->image))
          <img src="{{asset('uploads/images/feed/'.@$news_Feeds->image)}}" alt="" class="img-fluid uploadimg viewImagePreview editImagePreview">
        @else
          <img src="" alt="" class="img-fluid uploadimg viewImagePreview">
        @endif          
        <input type="hidden" name="image_name" class="imageName crop_img1" value="{{@$news_Feeds->image}}">
...
<div class="form-group text-center @if(empty(@$news_Feeds->image)){{ 'edit_image_preview' }}@endif ">
...
<div class="popFeedLink">
  <div class="form-group">
    <input id="link" type="text" name="link" class="form-control" value="{{@$news_Feeds->url}}" placeholder="Link" oninput="getOgProperty(this);">
    <span class="errText" style="display:none;"></span>
  </div>
</div>
<div class="popFeedTitle">
  <div class="form-group">
    <input id="Title" type="text" name="title" class="form-control" placeholder="Title" value="{{@$news_Feeds->title}}">
    <span class="errText" style="display:none;"></span>
  </div>
</div>
<div class="popFeedCaption">
  <div class="form-group">
    <textarea class="form-control" rows="3" id="Caption" placeholder="Caption" value="" name="caption">{{@$news_Feeds->caption}}</textarea>
  </div>
</div>
...
<script type="text/javascript">
  ...
  function crop_upload_image(e){
    ...
    $uploadCrop.croppie('result', {
      ...
      $.ajax({
        url: "{{ route('front.feeds_news.croppie-image-post') }}",
        type: "POST",
        data: formData,
        processData: false,
        contentType: false,
  ...
  function getOgProperty(e){
		var url = $(e).val();
		$.ajax({
			url: "{{route('front.feeds.getOgProperty')}}",
