                ...
                @if(count($navigations))
                    @foreach($navigations[0] as $groupId => $group) 
                        <div id="permission-user-{{ $groupId }}" class="col-sm-12">
                            ...
                            <div class="col-sm-8">        
                                <label>
                                    <input type="checkbox" id="chkAll_{{ $groupId }}" name="navigation_id[]" value="{{ $groupId }}" class="checkAll" {{ $groupId == 1 ? 'disabled' : '' }} {{ $groupId == 1 ? 'checked' : (in_array($groupId, $rolePermissions) ? 'checked' : '') }}> 
                                </label>  
                            </div>
                            ...
                            @if(isset($navigations[$groupId]) && count($navigations[$groupId]))
                                @foreach ($navigations[$groupId] as $navigation_id => $name)
                                    ...
                                    <div class="col-sm-2">
                                        <label>
                                            <input type="checkbox" id="chkAll_{{ $navigation_id }}" name="navigation_id[]" value="{{ $navigation_id }}" {{ in_array($navigation_id, $rolePermissions) ? 'checked' : '' }}> 
                                        </label>
                                    </div>
                                @endforeach
                            @endif
