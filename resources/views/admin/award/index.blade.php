...
@section('title') All Awards of {{$event->name}} @endsection

@section('content')

	<!-- Content Header (Page header) -->
        <section class="content-header">
            <h1> All Awards of {{$event->name}} </h1>
            <ol class="breadcrumb">
                <li><a href="{{ route('admin.dashboard') }}"><i class="fa fa-dashboard"></i> {{ adminTransLang('dashboard') }}</a></li>
                <li class="active"> All Awards of {{$event->name}} </li>
            </ol>
        </section>
        ...
        <section class="content">
            ...
            <p>
                 <a href="{{ route('admin.event.award.create') }}/{{$event->id}}" class="btn btn-success">Create awards</a>
            </p>
            <div class="row">
                <div class="col-md-12">
				    <div class="box">
...
<script type="text/javascript">
	$(function() {
        $('#navigation-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route("admin.event.award.list",$event->id) }}',
            columns : [
	            { "data": "name" },
	            { "data": "type" },
                {
