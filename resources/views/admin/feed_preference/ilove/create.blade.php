              ...
              <div class="box">
                ...
                <div class="box-body">
                    ...
                    <form class="form-horizontal" id="create-form" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="group_id" class="col-sm-2 control-label">Group Name <i class="has-error">*</i></label>
                            <div class="col-sm-6">
                               <select required="" class="form-control" name="group" id="group_id">
                                 <option value="">Select Group</option>
                                 @foreach(config('cms.group') as $key => $value)
                                 <option value="{{$key}}" @if(!empty($row->group) && $row->group == $key ) selected @endif>{{$value}}</option>
                                 @endforeach
                              </select>
                            </div>
                        </div>
                         <div class="form-group">
                            <label for="group_id" class="col-sm-2 control-label"> Category <i class="has-error">*</i></label>
                            <div class="col-sm-6">
                                 <select name="categories[]" id="category" data-no="1" class="custom-select get_sc form-control select2" data-placeholder="Select" multiple="">
                                 <option value="">Select Category</option>
                                 @php 
                                    @$categoriesArr = explode(',',@$row->categories);
                                    @endphp
                                 @foreach (category() as $index => $value)
                                 <option value="{{$index}}"  @if(in_array(@$index,@$categoriesArr)){{ "selected" }} @endif
                                 >{{$value}}</option>
                                 @endforeach
                              </select>
                            </div>
...
<script type="text/javascript">
    jQuery(function($) {
        ...
        $(document).on('submit','#create-form',function(e){
            ...
            processData: false,
            contentType: false,
            url: "{{ route('admin.feed_preference.ilove.create') }}",
            ...
            success: function (data)
            {
                ...
                if(data.status == 1)
                {
                    ...
                    window.location.replace('{{ route("admin.feed_preference.ilove.index")}}');
...
<script type="text/javascript">
   jQuery(document).on("change", "#group_id" , function() {
     ...
     $.ajax({
        type:'GET',
        url:"{{ url('admin/feed_preference/get_category_BYGroup') }}",
        data:{
             "_token": "{{ csrf_token() }}",
             "id": id,
         },
