  ...
  <!-- Content Header (Page header) -->
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ route('admin.blog.index') }}"> All Blog Category </a></li>
                ...
                <div class="box-body">
                    ...
                    <form class="form-horizontal" id="create-form" enctype="multipart/form-data">
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-6">
                          <input type="text" class="form-control" name="name" placeholder="Name" value="{{@$blog_category->name}}">
                        </div>
                      </div>
                        ...
                        <div class="col-sm-6">
                            <select class="form-control" name="status">
                              @foreach(config('cms.action_status') as $key => $status)
                                <option value="{{ $key }}" {{isset($blog_category) && $blog_category->status == $key ? 'selected' : ''}}>{{ $status }}</option>
                              @endforeach
                            </select>
                        </div>
                      </div>
    ...
    jQuery(function($) {
      $(document).on('click','#createBtn',function(e){
            ...
            $.ajax({
                processData: false,
                contentType: false,
                url: "{{ route('admin.blog_category.create') }}",
                ...
                success: function (data)
                {
                    ...
                    if(data.status == 1)
                    {
                        ...
                        window.location.replace('{{ route("admin.blog_category.index")}}');
