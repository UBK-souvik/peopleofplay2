...
<div class="form-group">
  ...
  <div class="col-sm-6">
    <select class="form-control select2" name="user_id" onchange="{{$str_author_onchange}}">
      ...
      @endphp							   
      <option value="{{ $key }}" {{isset($int_user_id_sel_new) && $int_user_id_sel_new == $key ? 'selected' : ''}}>{{ $status }}</option>
      @endforeach
    </select>
  </div>
</div>