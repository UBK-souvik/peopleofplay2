...
<div class="left-column border_right AccountSubscription">
    <div class="First-column bg-white p-3">
        ...
        <div class="row mt-3">
		   ...
		   <?php if(!empty($user->stripe_id) && strpos($user->stripe_id, 'us_')>0): ?>		
				<div class="col-md-12">
        	...
        	<div class="col-md-12">
        		<h5 class="text-left">Expiry Date - 
                    ...
                    <?php elseif(@$user->gold == 2): ?>
                        <span style="color: red;">Canceled Member</span>
                    <?php elseif(@$user->gold == 3): ?>
        		...
        		<table class="mt-3 w-100">
        			<tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo e(@$user->subscription->ends_at); ?></td>
                        </tr>
        				<!-- <tr>
        				...
        				<tr>
                            ...
                            <?php if(empty($subscription->cancelled_at) OR (!empty($subscription->cancelled_at) AND $now >=$date)): ?>
                            <?php if(@$user->gold == 1): ?>
				                <td colspan="2">
                            ...
                            <?php elseif(@$user->gold == 2): ?>
                                <td colspan="2">
                                    <p>You have canceled your subsription. Please click <a style="color: #0056b3;" href="<?php echo e(route('front.plans',@$plan_id)); ?>">here</a> to buy membership.</p>
                                </td>
                             <?php elseif(@$user->gold == 3): ?>
                                <td colspan="2">
                                    <p>Refunded</p>
...
<script type="text/javascript">
	...
	$.ajax({
                    type: "GET",
                    url: "<?php echo e(route('front.user.site.ajax.account.data')); ?>",
                    data: {
                      _token: CSRF_TOKEN, 
                    },
                    beforeSend: function() {
	...
	$(document).ready(function(){
		...
		$('#renew').click(function(){
            if (confirm('Are you sure?')) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo e(route('front.plan.renew')); ?>",
                    data: {
                      _token: CSRF_TOKEN, 
                    },
                    dataType: "json",
        ...
        $('#cancel').click(function(){
            if (confirm('Are you sure?')) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo e(route('front.cancel.subscription')); ?>",
                    data: {
                      _token: CSRF_TOKEN, 
                    },
                    dataType: "json",
...
form.addEventListener('submit', function (ev) {
    ...
    stripe
        .createPaymentMethod({
            ...
            card: card,
            billing_details: {
                name: '<?php echo e(get_current_user_info()->first_name); ?>',
            },
        })
        .then(function (result) {
          ...
          { 
            result._token = '<?php echo e(csrf_token()); ?>',
            $.ajax({
                type: "GET",
                url: "<?php echo e(route('front.card.update')); ?>",
                data: result,
                dataType: "json",
                beforeSend: function() {
                        $('#submit').attr('disabled',true);
