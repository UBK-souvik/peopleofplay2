...
?>
          ...
          <div class="First-column bg-white">
            <div class="col-md-12">
              <div class="row sectionTop">
                    <?php if( empty(get_current_user_info()->type_of_user) ): ?>
                        <div class="col-md-4 imgProfilePadding marginBottomTwenty lessMargin">
                            <!-- <div class="image-width justifly-content-center"> -->
                            <img src="<?php echo e(@prodEventImageBasePath(@$event->main_image)); ?>" class="img-fluid mr-0 imgtwoeighty">
                            <!-- </div> -->
                        </div>
                    <?php else: ?> 
                        <?php if(have_permission('event_p_p') ): ?>
                          <div class="col-md-4 imgProfilePadding marginBottomTwenty lessMargin">
                                <!-- <div class="image-width justifly-content-center"> -->
                                <img src="<?php echo e(@prodEventImageBasePath(@$event->main_image)); ?>" class="img-fluid mr-0 imgtwoeighty">
                                <!-- </div> -->
                   ...
                   <div class="col-md-8 rightBox">
                     <div class="Jengatext paragraph">
                       <h2 class="mb-1"> 
                          ...
                          <?php else: ?>
                            <?php if(have_permission('event_name') ): ?>
                                <?php echo e($event->name); ?>
                        ...
                        ?>
                        <!-- <p class="productAuthor">by <a class="span-style1" target="_blank" href="<?php echo e($str_user_url_new); ?>"><?php echo e($str_user_name); ?></a></p> -->
                     </div>
                     <div>
                       <p class="productAuthor m-0 fontWeightSix">by : <a class="span-style1 fontWeightSix" target="_blank" href="<?php echo e($str_user_url_new); ?>"><?php echo e($str_user_name); ?></a></p>
                        ...
                        <?php if( empty(get_current_user_info()->type_of_user) ): ?>
                           <?php echo $__env->make("front.user.modules.event_description", array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
						            <?php else: ?> 
                          <?php if(have_permission('event_description') ): ?>
                            <?php echo $__env->make("front.user.modules.event_description", array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>
                          <?php endif; ?>
                        <?php endif; ?>
                        <?php if(!empty(@$event->fun_fact1) || !empty(@$event->fun_fact2) || !empty(@$event->fun_fact2) ): ?>
                            <p>Fun Facts: </p>
                            <ul style="list-style: inside; color: #111;">
                              <?php if(!empty(@$event->fun_fact1) ): ?><li class="p-text"><?php echo e(@$event->fun_fact1); ?></li> <?php endif; ?>
                              <?php if(!empty(@$event->fun_fact2) ): ?><li class="p-text"><?php echo e(@$event->fun_fact2); ?></li> <?php endif; ?>
                              <?php if(!empty(@$event->fun_fact3) ): ?><li class="p-text"><?php echo e(@$event->fun_fact3); ?></li> <?php endif; ?>
                            </ul>
                            <hr>
                        <?php endif; ?>
                        <?php if( empty(get_current_user_info()->type_of_user) ): ?>
                             <p class="sidetitleProduct m-0">Website: <a href="<?php echo e($event->website); ?>"><span class="span-style text-lowercase"><?php echo e($event->website); ?></span></a></p>
                        <?php else: ?>
                        ...
                        <?php if(Auth::guard('users')->check()): ?>
                          <?php if(check_watch_list(3,$event->id)): ?>
                              <a type="button" href="#" class="btn NoPaddingWatch mt-2">
                                <i class="fa fa-check photo_icon" ></i>Added to Watchlist
                              </a>
                          <?php else: ?>
                              <a type="button" href="<?php echo e(route('front.pages.add_to_watch_list')); ?>?type=3&value=<?php echo e($event->id); ?>" class="btn NoPaddingWatch mt-2"><i class="fa fa-plus photo_icon"></i> Add to Watchlist</a>
                          <?php endif; ?>
                        <?php endif; ?>
                   </div>
              </div>
    ...
    <?php endif; ?>
		  ...
		  <?php if( (!empty($event->awards) && count($event->awards)>0)): ?>
                ...
                <div class="row sectionBox">
                  ...
                  <div>
                    <table class="table event_table short_collaborator_list">
                        <tbody>
                            <?php $__currentLoopData = $event->awards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $collaborator_key => $award): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                              <?php if($collaborator_key <= 2 ): ?>
                                <tr class="py-1 px-3">
                                  <td class="eve_dac_name"><?php echo e($award->name); ?></td>
                                  <td>...</td>
                                  <td>
                                      <a class="span-style1" href="<?php echo e(route('front.user.awardnominee.index')); ?>?id=<?php echo e($award->id); ?>">View</a>
                                  </td>
                                </tr>
                              <?php endif; ?>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tbody>
                        <?php if(count($event->awards) > 3 ): ?>
                            <tbody id="dots"></tbody>
                        <?php endif; ?>
                        <?php if(count($event->awards) > 3 ): ?>
                          <tbody id="more">
