...
class EntertainmentController extends Controller
{
   ...
   public function getCast()
    {
        $data = EntertainmentCategory::join('entertainments','entertainments.category_id','=','entertainment_categories.id')->select('entertainment_categories.*')->where('entertainment_categories.status',1)->where('entertainment_categories.type','cast')->groupBy('entertainment_categories.id')->get();
         foreach ($data as $key => $row) {
            $data[$key]->wiki = Entertainment::where(['status'=>1,'category_id'=>$row->id])->orderBy('id','DESC')->get()->take(6);
         }
