...
class SectionprofileController extends Controller
{
    ...
    {
// echo 111;exit;
        return view('admin.sectionprofile.index');
    }
    ...
    public function getList()
    {
        ...
        $section_profile = \App\Models\SectionProfile::select('*');
        // print_r($section_profile);exit;
        return \DataTables::of($section_profile)
    ...
    public function getCreate()
    {
		...
		return view('admin.sectionprofile.create', compact('users'));
    }

    public function postCreate(Request $request)
    {
        // echo 111;exit;
            $section_profile = new SectionProfile();
        ...
        try {
            ...
            if ($request->filled('profile_id')) {
                    $section_profile = SectionProfile::findOrFail($request->profile_id);
                }
                ...
                // Shubham Code For Image Compression Start //
                    ...
                    if($img->resize($image_comp_size[0], $image_comp_size[1], function ($constraint) {
                        ...
                        $constraint->aspectRatio();
                            ...
                            $section_profile->main_image = $filename;
                    }else{
            ...
            $section_profile->profileName = $request->profileName;
            $section_profile->profileSubtitle = $request->profileSubtitle;
            $section_profile->profileUrl = $request->profileUrl;

                // print_r ($section_profile);exit;
            $section_profile->save();
    ...
    public function getUpdate($id)
    {
        ...
        $section_profile  = SectionProfile::findOrFail($id);
        // echo "<pre>";
        // print_r($section_profile);exit;
        // $users = User::pluck('email', 'id');
        //$users = User::where('role', '!=', 1)->get();
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.sectionprofile.create', compact('section_profile'));
    }
    ...
    public function getView($id)
    {
        ...
        //$users = User::pluck('email', 'id');
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.sectionprofile.view', compact('section_profile'));
    }

    public function getDelete($id)
    {
        $section_profile  = SectionProfile::findOrFail($id)->delete();
        return response()->json([
            'status' => 1,
            'msg' => adminTransLang('request_processed_successfully'),
        ], 200);
