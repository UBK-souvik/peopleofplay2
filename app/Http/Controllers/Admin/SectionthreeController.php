...
class SectionthreeController extends Controller
{
    ...
    {
// echo 111;exit;
        return view('admin.sectionthree.index');
    }
    ...
    public function getList()
    {
        ...
        $sectionthree_header = \App\Models\SectionThree::select('*');
        // print_r($sectionthree_header);exit;
        return \DataTables::of($sectionthree_header)
    ...
    public function getCreate()
    {
		...
		return view('admin.sectionthree.create', compact('users'));
    }

    public function postCreate(Request $request)
    {
        // echo 111;exit;
            $sectionthree_header = new SectionThree();
        ...
        try {
            ...
            if ($request->filled('header_id')) {
                    $sectionthree_header = SectionThree::findOrFail($request->header_id);
                }

            $sectionthree_header->profileHeader = $request->headerName;

                // print_r ($sectionthree_header);exit;
            $sectionthree_header->save();
    ...
    public function getUpdate($id)
    {
        ...
        $sectionthree_header  = SectionThree::findOrFail($id);
        // echo "<pre>";
        // print_r($sectionthree_header);exit;
        // $users = User::pluck('email', 'id');
        //$users = User::where('role', '!=', 1)->get();
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.sectionthree.create', compact('sectionthree_header'));
    }
    ...
    public function getView($id)
    {
        ...
        //$users = User::pluck('email', 'id');
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.sectionthree.view', compact('sectionthree_header'));
    }

    public function getDelete($id)
    {
        $sectionthree_header  = SectionThree::findOrFail($id)->delete();
        return response()->json([
            'status' => 1,
            'msg' => adminTransLang('request_processed_successfully'),
        ], 200);
