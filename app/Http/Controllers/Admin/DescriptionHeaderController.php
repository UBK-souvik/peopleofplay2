...
class DescriptionHeaderController extends Controller
{
    public function getIndex()
    {
// echo 111;exit;
        return view('admin.descriptionheader.index');
    }
    ...
    public function getList()
    {
        ...
        $description_header = \App\Models\DescriptionHeader::select('*');
        // print_r($profile_header);exit;
        return \DataTables::of($description_header)
    ...
    public function getCreate()
    {
		...
		return view('admin.descriptionheader.create', compact('users'));
    }

    public function postCreate(Request $request)
    {
        // echo 111;exit;
            $description_header = new DescriptionHeader();
        ...
        try {
            ...
            if ($request->filled('header_id')) {
                    $description_header = DescriptionHeader::findOrFail($request->header_id);
                }

            $description_header->description_main_header = $request->headerName;

                // print_r ($profile_header);exit;
            $description_header->save();
    ...
    public function getUpdate($id)
    {
        ...
        $description_header  = DescriptionHeader::findOrFail($id);
        // echo "<pre>";
        // print_r($profile_header);exit;
        // $users = User::pluck('email', 'id');
        //$users = User::where('role', '!=', 1)->get();
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.descriptionheader.create', compact('description_header'));
    }
    ...
    public function getView($id)
    {
        ...
        //$users = User::pluck('email', 'id');
		// $users = User::get_all_user_list_by_email_name();
        return view('admin.descriptionheader.view', compact('description_header'));
    }

    public function getDelete($id)
    {
        $description_header  = DescriptionHeader::findOrFail($id)->delete();
        return response()->json([
            'status' => 1,
            'msg' => adminTransLang('request_processed_successfully'),
        ], 200);
