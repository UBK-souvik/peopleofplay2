...
class CastCategoryController extends Controller
{
    public function getIndex()
    {
        ...
        return view('admin.cast.category.index');
    }

    public function getList()
    {
        $news_category = \App\Models\EntertainmentCategory::select('*')->where('type','cast');
        return \DataTables::of($news_category)
            ->addColumn('action', function ($query) {
                $disabledCheck = EntertainmentCategory::checkDelete($query->id);
                $disabled ='';
                $delete_url = URL::to("admin/cast/category/delete/".$query->id);
                if($disabledCheck == 1)
                {
                   $delete_url = 'javascript:void(0);';
                    $disabled ='disabled';
                } 
                return '<a href="'. URL::to("admin/cast/category/update") .'/'.$query->id.'">
    ...
    public function getCreate()
    {
        ...
        return view('admin.cast.category.create');
    }
    ...
    public function getUpdate($id)
    {
        ...
        return view('admin.cast.category.create', compact('data'));
    }
