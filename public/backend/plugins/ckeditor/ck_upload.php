...
$upload_dir = array( 
    'img'=> '/uploads/images/editor_images/', 
); 
...
$str_new_number = $int_time_stamp . '_' . $int_rand_num;
...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) { 
    ...
    define('F_NAME', preg_replace('/\.(.+?)$/i', '', $str_new_number . '_' . basename($_FILES['upload']['name'])));   
    ...
    if($re == ''){ 
        if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) { 
            ...
            $url = $_SERVER['APP_URL'] . '/'. $upload_dir . $f_name; 
